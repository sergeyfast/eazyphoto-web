<?php
    ...
    JsHelper::PushLine(
        sprintf( "var objectBasePath = '%s', objectDeleteStr = '%s';",
            Site::GetWebPath( 'vt://users/' ),
            T( 'vt.user.deleteString' )
        )
    );

    $__pageTitle   = T( 'vt.screens.user.editTitle' );
    $__breadcrumbs = [
        [ 'link' => Site::GetWebPath( 'vt://users/' ), 'title' => T( 'vt.screens.user.list' ) ],
        [ 'link' => Site::GetWebPath( 'vt://users/edit/' . $objectId ), 'title' => T( 'vt.common.crumbEdit' ) ],
    ];
?>
...
<main role="main">
    ...
    <div class="container"><a href="{web:vt://users/}" class="linkBlock cLink fsBigX floatRight">← <span class="link">{lang:vt.common.back}</span></a>
        <h1>{$__pageTitle}</h1>
        <form method="post" action="{web:vt://users/}edit/{$objectId}" enctype="multipart/form-data" id="data-form" data-object-id="{$objectId}" >
