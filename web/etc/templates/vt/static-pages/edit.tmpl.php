<?php
    ...
    JsHelper::PushLine(
        sprintf( "var objectBasePath = '%s', objectDeleteStr = '%s';",
            Site::GetWebPath( 'vt://static-pages/' ),
            T( 'vt.staticPage.deleteString' )
        )
    );

    $__pageTitle   = T( 'vt.screens.staticPage.editTitle' );
    $__breadcrumbs = [
        [ 'link' => Site::GetWebPath( 'vt://static-pages/' ), 'title' => T( 'vt.screens.staticPage.list' ) ],
        [ 'link' => Site::GetWebPath( 'vt://static-pages/edit/' . $objectId ), 'title' => T( 'vt.common.crumbEdit' ) ],
    ];
?>
...
<main role="main">
    ...
    <div class="container"><a href="{web:vt://static-pages/}" class="linkBlock cLink fsBigX floatRight">← <span class="link">{lang:vt.common.back}</span></a>
        <h1>{$__pageTitle}</h1>
        <form method="post" action="{web:vt://static-pages/}edit/{$objectId}" enctype="multipart/form-data" id="data-form" data-object-id="{$objectId}" >
