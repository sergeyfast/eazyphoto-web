<?php
    ...
    JsHelper::PushLine(
        sprintf( "var objectBasePath = '%s', objectDeleteStr = '%s';",
            Site::GetWebPath( 'vt://photos/' ),
            T( 'vt.photo.deleteString' )
        )
    );

    $__pageTitle   = T( 'vt.screens.photo.editTitle' );
    $__breadcrumbs = [
        [ 'link' => Site::GetWebPath( 'vt://photos/' ), 'title' => T( 'vt.screens.photo.list' ) ],
        [ 'link' => Site::GetWebPath( 'vt://photos/edit/' . $objectId ), 'title' => T( 'vt.common.crumbEdit' ) ],
    ];
?>
