    ...
    $grid = array(
        "columns" => array(
           LocaleLoader::Translate( "vt.album.title" )
            ...
            , LocaleLoader::Translate( "vt.album.isPrivate" )
            , LocaleLoader::Translate( "vt.album.startDate" )
            , LocaleLoader::Translate( "vt.album.roSecret" )
            , LocaleLoader::Translate( "vt.album.statusId" )
        )
        , "colspans"	=> array()
        , "sorts"		=> array(0 => "title", 1 => "alias", 2 => "isPrivate", 3 => "startDate", 4 => "roSecret", 5 => "statusId")
        , "operations"	=> true
        , "allowAdd"	=> true
		...
		<div class="search<?= $hideSearch == "true" ? " closed" : ""  ?>">
			...
			<form class="search-form" id="searchForm" method="post" action="{$grid[basepath]}">
                ...
                <div class="row">
                    ...
                    <?= FormHelper::FormInput( "search[roSecret]", $search['roSecret'], 'roSecret', null, array( 'size' => 80 ) ); ?>
                </div>
                <div class="row">
                    <label>{lang:vt.album.statusId}</label>
                    <?= FormHelper::FormSelect( "search[statusId]", StatusUtility::$Album[$__currentLang], "", "", $search['statusId'], null, null, true ); ?>
                </div>
...
?>
			<tr data-object-id="{$id}">
                ...
                <td>{$object.startDate.DefaultDateFormat()}</td>
                <td>{$object.roSecret}</td>
