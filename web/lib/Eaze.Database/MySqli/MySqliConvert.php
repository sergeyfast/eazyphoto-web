<?php
    /**
     * MySQLi Type Converter
     * 
     * @package Eaze
     * @subpackage MySqli
     * @author sergeyfast
     */
    class MySqliConvert implements ISqlConvert {
        ...
        public static function ToString( $value ) {
            ...
            if ( empty( self::$connection ) || !self::$connection->IsOpened() ) {
                Logger::Error( 'Connection was not found ' );
                return null;
            }

            /** @var $resource mysqli */
            $resource  = self::$connection->GetResource();
            $sqlString = "'" . $resource->escape_string( $value ) . "'";
        ...
        public static function FromParameter( $value, $type ) {
            switch ( $type ) {
                ...
                case TYPE_LTREE:
                    return self::FromString( $value );
                default:
                    Logger::Error( 'Cannot call converter for %s of class MySqliConvert', $type);
                    return null;
            }
        }
